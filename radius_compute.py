...
CITY_RADIUS_FEATURES_PATH = "./saved_data_spb_radius"
...
pool = Pool(56, initializer=init_netatmo, initargs=(basic_net_data, block_ids, netatmo))
feat_gen = pool.imap(get_netatmo_features, gen_gps_hour(X, block_ids))

all_feat = list(tqdm(feat_gen))
all_feat_pd = pd.DataFrame(all_feat)
